...
archs_option=("--replace-archs" x86_64 i386)
...
xcode_default_sdk_path="$xcode_developer_path/Platforms/iPhoneSimulator.platform/Developer/SDKs/iPhoneSimulator.sdk"

xcode_sdk_paths="$xcode_developer_path/Platforms/iPhoneSimulator.platform/Developer/SDKs/*"
...
xcode_sdk_ios_version=${preferred_xcode_sdk_name:15} # Remove 'iPhoneSimulator' in front of sdk name
...
if [ -d "$device_support_dir" ] && [ "$use_simulator" == "-" ]; then
    for symbols_path in "$device_support_dir"*; do
        ...
        sdk_name=$(printf "iPhoneSimulator%s.sdk" $ios_version)
    ...
    ios_version=$xcode_sdk_ios_version
    sdk_name=$(printf "iPhoneSimulator%s.sdk" $ios_version)
    ...
    parse_paths=("-p" "-r" "all" $no_warnings "${tbd_options[@]}" "${archs_option[@]}" -v$version "$xcode_sim_runtime_path/Developer"
                 "-p" "-r" "all" $no_warnings "${tbd_options[@]}" "${archs_option[@]}" -v$version "$xcode_sim_runtime_path/System"
                 "-p" "-r" "all" $no_warnings "${tbd_options[@]}" "${archs_option[@]}" -v$version "$xcode_sim_runtime_path/Library")
